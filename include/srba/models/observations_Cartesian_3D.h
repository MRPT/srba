...
namespace observations {
	...
	template <> struct landmark_matcher<Cartesian_3D>
	{
		...
		static bool find_relative_pose(
			const mrpt::aligned_std_vector<Cartesian_3D::obs_data_t> & new_kf_obs,
			const mrpt::aligned_std_vector<Cartesian_3D::obs_data_t> & old_kf_obs,
			...
			POSE &pose_new_kf_wrt_old_kf)
		{
			...
			for (size_t i=0;i<N;i++)
				matches.emplace_back(i,i, old_kf_obs[i].pt.x,old_kf_obs[i].pt.y,old_kf_obs[i].pt.z,  new_kf_obs[i].pt.x,new_kf_obs[i].pt.y,new_kf_obs[i].pt.z  );
