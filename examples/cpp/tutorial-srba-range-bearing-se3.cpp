...
//	typedef options::sensor_pose_on_robot_none      sensor_pose_on_robot_t;  //!< The sensor pose coincides with the robot pose
	typedef options::observation_noise_constant_matrix<observations::RangeBearing_3D>   obs_noise_matrix_t;      // The sensor noise matrix is the same for all observations and equal to some given matrix
...
//	rba.parameters.obs_noise.std_noise_observations = 0.03; //SENSOR_NOISE_STD;
	...
	for (size_t i=0;i<sizeof(observations_0)/sizeof(observations_0[0]);i++)
	{
		...
		obs_field.obs.obs_data.pitch = observations_0[i].pitch + getRandomGenerator().drawGaussian1D(0,SENSOR_NOISE_STD);
	...
	for (size_t i=0;i<sizeof(observations_10)/sizeof(observations_10[0]);i++)
	{
		...
		obs_field.obs.obs_data.pitch = observations_10[i].pitch + getRandomGenerator().drawGaussian1D(0,SENSOR_NOISE_STD);
		list_obs.push_back( obs_field );
	}
